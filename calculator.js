...
document.onkeyup = function(event) {
	...
	if (/[0-9]/.test(key)) {
		keyboardInput.innerText += key;
	} else if (key === '.') {
		keyboardInput.innerText += '.';
	} else if (key === '+') {
		keyboardInput.innerText += '+';
	} else if (key === '-') {
		keyboardInput.innerText += '-';
		...
		const expression = keyboardInput.innerText;
		const result = eval(expression);
		keyboardInput.innerText = result;
	} else if (key === 'Escape') {
		keyboardInput.innerText = '';
	} else if (key === 'Backspace') {
		const currentInput = keyboardInput.innerText;
		keyboardInput.innerText = currentInput.slice(0, -1);
	}
};
